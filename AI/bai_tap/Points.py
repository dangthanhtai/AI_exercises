...
class Zone_3D:
    def __init__(self, n_points, n_classes, n_dimensions, centers):
        ...
        for j in range(self.K):
            ...
            k = np.zeros(self.N)
            r = np.zeros(self.N)
            g = np.linspace(0, 2 * np.pi, self.N) + np.random.randn(self.N) * 0.2  # gamma
            t = np.linspace(0, 2 * np.pi, self.N) + np.random.randn(self.N) * 0.2  # theta
            for i in range(len(R)):
                R[i] = np.random.randn(1) * 2
                k[i] = R[i] * np.sin(g[i])
                r[i] = R[i] * np.cos(g[i])
            self.P[ix] = np.c_[center_j[0] + r * np.sin(t),
                               center_j[1] + r * np.cos(t),
                               center_j[2] + k]
            self.L[ix] = j
